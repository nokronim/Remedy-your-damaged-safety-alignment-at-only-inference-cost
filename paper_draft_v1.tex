...
\paragraph{Training Data Composition.} Table~\ref{tab:ablation_data} examines which categories of training domains are most important for the hypernetwork. Removing instruction-following domains (6 domains) causes a dramatic shift: the classifier defaults to ``safe,'' achieving near-perfect task preservation but losing 8+ points of safety recovery. This reveals that instruction-following data---which contains the most diverse prompt formats and response patterns---provides the signal needed to learn the safe-vs-harmful boundary. Removing QA or math/code domains has minimal impact, confirming that the hypernetwork is robust to moderate reductions in training diversity as long as instruction-following data is retained.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Training Configuration} & \textbf{\#Dom.} & \textbf{Task Pres.} & \textbf{Safety Rec.} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Effect of training data composition. Instruction-following domains are critical for safety discrimination; other categories can be removed with minimal impact. Excluding 1--2 individual domains shows robustness.}
\label{tab:ablation_data}
\end{table}

\subsection{Analysis}

\paragraph{Safety Degradation Visualization.} Figure~\ref{fig:safety_degrad} illustrates the safety degradation pattern across all 12 holdout domains. Fine-tuning on instruction-following and commonsense domains causes the most severe safety degradation (up to 43\% harmful rate), while math and QA domains show milder effects. \method{} consistently restores the harmful rate to near zero regardless of the degradation severity.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/safety_degradation.pdf}
    \caption{Harmful output rate across domains. LoRA fine-tuning degrades safety (red); \method{} restores it (blue, near zero).}
    \label{fig:safety_degrad}
\end{figure}

\paragraph{Accuracy Preservation.} Figure~\ref{fig:acc_pres} plots SafeHead accuracy against LoRA accuracy for all 12 holdout domains. Points cluster tightly along the $y=x$ diagonal, with green markers indicating cases where SafeHead slightly \textit{improves} over LoRA. The maximum deviation is 0.6 percentage points.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/accuracy_preservation.pdf}
    \caption{SafeHead vs.\ LoRA accuracy across all holdout domains. Points on or above the diagonal indicate preserved or improved accuracy. Green = improvement; blue = minor drop.}
    \label{fig:acc_pres}
\end{figure}

\paragraph{Ablation Visualization.} Figure~\ref{fig:ablation_comb} summarizes the component and data composition ablations. Each component contributes to the overall system; safe-probe supervision and instruction-following training data are the most critical.

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/ablation_combined.pdf}
    \caption{Ablation studies. (a) Removing any loss component degrades either task preservation or safety recovery. (b) Instruction-following training data is essential for safety; other categories can be removed with minimal impact.}
    \label{fig:ablation_comb}
\end{figure*}
