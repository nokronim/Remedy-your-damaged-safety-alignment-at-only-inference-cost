...
\end{equation}
where $\sigma$ is the sigmoid function. At inference time, $\lambda > 0.5$ flags the prompt as harmful; for safe prompts ($\lambda \leq 0.5$), the side network is bypassed entirely and the base model output is used directly, preserving task performance.
