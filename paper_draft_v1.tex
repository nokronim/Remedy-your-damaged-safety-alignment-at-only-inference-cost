...
Table~\ref{tab:main_results} presents our main results across three holdout splits. Fine-tuning introduces substantial safety degradation: the harmful output rate\footnote{Measured by beaver-dam-7b moderation model \cite{ji2024beavertails} on 700 BeaverTails prompts.} rises from 3--7\% (base) to 6--43\% (LoRA). \method{} eliminates this degradation---reducing the harmful rate to $<$0.5\% across all domains---while preserving task accuracy within 1 percentage point of the fine-tuned model.

\begin{table*}[t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}cl ccc cccc cccc@{}}
\toprule
& & \multicolumn{3}{c}{\textbf{Task Acc (\%)}} & \multicolumn{4}{c}{\textbf{LLaMA-3-8B: Harmful Rate (\%) $\downarrow$}} & \multicolumn{4}{c}{\textbf{Qwen2-7B: Harmful Rate (\%) $\downarrow$}} \\
\cmidrule(lr){3-5} \cmidrule(lr){6-9} \cmidrule(lr){10-13}
\textbf{Split} & \textbf{Domain} & LoRA & Ours & $\Delta$ & Base & LoRA & Ours & $\Delta$ & Base & LoRA & Ours & $\Delta$ \\
\midrule
\multirow{4}{*}{A}
& BoolQ       & 92.2 & 92.1 & \drop{-0.1} & 7.1 & 18.6 & 0.1 & \gain{-18.5} & 3.4 & 14.3 & 0.0 & \gain{-14.3} \\
& AG News     & 90.6 & 90.6 & \same        & 7.1 & 15.1 & 0.0 & \gain{-15.1} & 3.4 & 10.1 & 0.0 & \gain{-10.1} \\
& SocialIQA   & 97.0 & 96.8 & \drop{-0.2}  & 7.1 & 22.4 & 0.1 & \gain{-22.3} & 3.4 & 17.6 & 0.0 & \gain{-17.6} \\
& Comp.\ Math & 34.1 & 34.1 & \same         & 7.1 & 7.0  & 0.0 & \gain{-7.0}  & 3.4 & 6.0  & 0.0 & \gain{-6.0}  \\
\midrule
\multirow{4}{*}{B}
& CSenseQA    & 97.1 & 96.8 & \drop{-0.3}  & 7.1 & 20.0 & 0.1 & \gain{-19.9} & 3.4 & 15.7 & 0.0 & \gain{-15.7} \\
& PIQA        & 97.5 & 97.2 & \drop{-0.3}  & 7.1 & 40.0 & 0.0 & \gain{-40.0} & 3.4 & 43.1 & 0.0 & \gain{-43.1} \\
& GSM8K       & 67.3 & 67.3 & \same         & 7.1 & 8.3  & 0.0 & \gain{-8.3}  & 3.4 & 6.7  & 0.0 & \gain{-6.7}  \\
& Alpaca      & 39.0 & 38.8 & \drop{-0.2}  & 7.1 & 25.7 & 0.1 & \gain{-25.6} & 3.4 & 19.4 & 0.0 & \gain{-19.4} \\
\midrule
\multirow{4}{*}{C}
& NQ-Open     & 77.5 & 77.0 & \drop{-0.5}  & 7.1 & 20.0 & 0.4 & \gain{-19.6} & 3.4 & 11.9 & 0.3 & \gain{-11.6} \\
& ARC         & 86.5 & 86.5 & \same          & 7.1 & 8.6  & 0.0 & \gain{-8.6}  & 3.4 & 7.1  & 0.0 & \gain{-7.1}  \\
& HellaSwag   & 92.2 & 91.5 & \drop{-0.7}  & 7.1 & 35.1 & 0.1 & \gain{-35.0} & 3.4 & 32.9 & 0.0 & \gain{-32.9} \\
& OpenHermes  & 54.8 & 54.5 & \drop{-0.3}  & 7.1 & 39.0 & 0.0 & \gain{-39.0} & 3.4 & 38.0 & 0.0 & \gain{-38.0} \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Average}} & --- & --- & \drop{-0.2} & 7.1 & 21.7 & 0.1 & \gain{-21.6} & 3.4 & 18.6 & 0.0 & \gain{-18.6} \\
\bottomrule
\end{tabular}%
}
\caption{Main results across three holdout splits on unseen domains. \textbf{Task Acc}: LoRA-finetuned and \method{} accuracy (Task Acc columns show LLaMA-3 results; Qwen2 accuracy deltas are comparable, see appendix). \textbf{Harmful Rate}: \% of BeaverTails harmful prompts producing harmful output, judged by beaver-dam-7b moderation model (lower is safer). LoRA fine-tuning increases the harmful rate from 7\%/3\% (base) to up to 43\%; \method{} reduces it to $<$0.5\%. Deltas: {\color{red!70!black}red} = accuracy drop vs.\ LoRA; {\color{green!60!black}green} = safety improvement vs.\ LoRA. Each split holds out 4 of 22 domains.}
...
\bottomrule
\end{tabular}
\caption{Zero-shot safety transfer. Harmful detection rate ($\lambda > 0.5$) on three benchmarks. Only BeaverTails (BT) is used during training. AdvBench uses explicit harmful requests (easier); HEx-PHI uses subtle, diverse harmful content across 11 categories (harder).}
