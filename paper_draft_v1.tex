...
\subsection{Side Network Training}
\label{sec:side_training}

Before hypernetwork training, we first obtain ground-truth side networks for each fine-tuning domain. For domain $d$, we train a side network alongside the frozen fine-tuned backbone $\mathcal{M}_d$ using a mixture of domain-specific safe data and harmful data from BeaverTails \cite{ji2024beavertails} (category-balanced, $\sim$7k samples).
...
\end{itemize}
The three losses are complementary: $\mathcal{L}_\text{recon}$ regularizes the high-dimensional weight space, while $\mathcal{L}_\text{lambda}$ and $\mathcal{L}_\text{func}$ ensure behavioral correctness on seen and unseen prompts respectively. We ablate each component in Section~\ref{sec:experiments}. We set $\alpha = 10$ and $\beta = 5$ to approximately equalize gradient magnitudes across the three losses. Training runs for 300 epochs with learning rate $5 \times 10^{-5}$ and hidden dimension 512.
...
\paragraph{Loss Components.} We ablate each loss term by training with it removed. Dropping $\mathcal{L}_\text{recon}$ tests whether behavioral supervision alone can guide the high-dimensional weight generation; dropping $\mathcal{L}_\text{func}$ tests whether calibration-prompt supervision suffices without held-out probe generalization; dropping $\mathcal{L}_\lambda$ tests whether weight reconstruction plus functional loss can implicitly learn the classifier (Table~\ref{tab:ablations}).
...
\addlinespace
\textit{Loss ablation} \\
\quad Full ($\mathcal{L}_\text{recon} + \mathcal{L}_\lambda + \mathcal{L}_\text{func}$) & ---  & ---  & ---  & --- \\
\quad $-\,\mathcal{L}_\text{recon}$ (behavioral only) & ---  & ---  & ---  & --- \\
\quad $-\,\mathcal{L}_\text{func}$  & ---  & ---  & ---  & --- \\
\quad $-\,\mathcal{L}_\lambda$      & ---  & ---  & ---  & --- \\
\bottomrule
\end{tabular}
