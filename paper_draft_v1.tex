...
Tables~\ref{tab:split_b} and~\ref{tab:split_a} show results on Split B and Split A holdout domains, respectively. The \method{} safety head reduces harmful flagging from 6--43\% (LoRA-only) to near 0\% across all domains, while preserving task accuracy within 1\% of the LoRA-only model.
...
\caption{\textbf{Split B} (unseen holdout domains). \method{} recovers safety (BT Flagged $\to$ 0\%) while maintaining LoRA-level task accuracy.}
\label{tab:split_b}
\end{table}
...
\caption{\textbf{Split A} (unseen holdout domains). \remark{TO FILL}}
\label{tab:split_a}
\end{table}
